  ...
  return (
    <header className="sticky top-0 z-30 flex h-14 min-w-0 max-w-full items-center justify-between gap-2 overflow-hidden border-b border-border/40 bg-background/80 px-2 backdrop-blur-xl sm:h-16 sm:px-4 md:px-6">
      <div className="flex items-center gap-1.5 sm:gap-3 flex-1 min-w-0">
        ...
        <div className={`relative min-w-0 flex-1 max-w-md hidden sm:block ${location.pathname.startsWith("/biblioteca") ? "sm:hidden" : ""}`} ref={searchRef}>
          ...
          <Input
            ...
            className="w-full h-8 sm:h-10 border-border/50 bg-secondary pl-8 sm:pl-9 pr-8 text-xs sm:text-sm placeholder:text-muted-foreground/60 focus-visible:ring-primary"
          />
          {searchTerm && (
            <button 
